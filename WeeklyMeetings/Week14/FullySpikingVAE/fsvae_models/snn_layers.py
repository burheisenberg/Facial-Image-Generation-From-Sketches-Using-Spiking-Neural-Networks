...
class tdLinear(nn.Linear):
    def __init__(self, 
        ...
        assert type(out_features) == int, 'outFeatures should not be more than 1 dimension. It was: {}'.format(out_features.shape)
...
class tdUpsample(nn.Upsample):
    def __init__(self, 
        ...
        # scale_factor
            ...
            scale_factor = (scale_factor[0], scale_factor[1], 1)
        else:
            raise Exception('Output shape is unknown. Given size: {0} and scale_factor: {1}'.format(size, scale_factor))
            
        # mode
        if mode not in ['nearest', 'linear', 'bilinear', 'bicubic', 'trilinear']:
            raise Exception('mode can be nearest, linear, bilinear, bicubic or trilinear. It was: {}'.format(mode))

        # align_corners
        if not isinstance(align_corners, bool):
            raise Exception('align_corners must be a boolean value. It was: {}'.format(align_corners))

        # recompute scale factor
        if not isinstance(recompute_scale_factor, bool):
            raise Exception('recompute_scale_factor must be a boolean value. It was: {}'.format(recompute_scale_factor))

        super().__init__(size=size, scale_factor=scale_factor, mode=mode, align_corners=align_corners)

    def forward(self, x):
        time_dim = x.shape[-1]  # get the time dimension & keep it constant
        self.size = (self.size[0], self.size[1], time_dim)
